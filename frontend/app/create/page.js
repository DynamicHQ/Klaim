...
import { createNFTOnServer, uploadToCloudinary, getConnectedWallet, initializeStorage } from '@/utils/mockData';
...
export default function CreateNFT() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Create NFT on server (with fallback to local storage)
      const result = await createNFTOnServer({
        name: formData.title,
      ...
      setTransactionResult({
        nftId: result.nftId || result.id,
        transactionHash: result.transactionHash || 'local-tx-' + Date.now(),
        imageUrl
      });
      setStatus('success');
        ...
        <div className="max-w-2xl mx-auto">
          <div className="text-center mb-8">
            ...
            <p className="text-base-content/70">
              Upload your art and mint it as an NFT
            </p>
          </div>
          ...
          {status === 'success' && transactionResult ? (
            <div className="card bg-base-100 shadow-xl">
              <div className="card-body text-center">
                ...
                {transactionResult.imageUrl && (
                  <img 
                    src={transactionResult.imageUrl} 
                    alt="Created NFT" 
                ...
                <div className="space-y-2 text-left">
                  ...
                  {transactionResult.imageUrl && (
                    <p><strong>Image URL:</strong> 
                      <a href={transactionResult.imageUrl} target="_blank" rel="noopener noreferrer" className="link link-primary ml-2">
                        View Image
                      </a>
                    </p>
                  )}
              ...
              <div className="card-body">
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    >
                      {status === 'loading' ? (
                        <>
                          ...
                          {uploading ? 'Uploading to Cloudinary...' : 'Creating NFT...'}
                        </>
                      ) : (
                        'Create NFT'
                      )}
                  ...
                  {status === 'loading' && (
                    <div className="text-center">
                      <p className="text-base-content/70">
                        {uploading ? 'Uploading image to Cloudinary...' : 'Creating your NFT...'}
                      </p>
