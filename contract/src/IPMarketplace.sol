...
contract IPMarketplace is ReentrancyGuard {
    ...
    function listIP(uint256 tokenId, uint256 price) external {
        require(IERC721(NFT_CONTRACT).ownerOf(tokenId) == msg.sender, "Not NFT owner");
        require(price > 0, "Invalid price");
        
        // Get IP ID from Story Protocol registry
        address ipId = IP_ASSET_REGISTRY.ipId(block.chainid, NFT_CONTRACT, tokenId);
        require(ipId != address(0), "IP not registered");
        
        // Generate unique listing ID
        bytes32 listingId = keccak256(abi.encodePacked(NFT_CONTRACT, tokenId, msg.sender, block.timestamp));
    ...
    function purchaseIP(bytes32 listingId) external nonReentrant {
        ...
        require(IERC721(NFT_CONTRACT).ownerOf(listing.tokenId) == listing.seller, "Seller no longer owns NFT");
        ...
        IERC721(NFT_CONTRACT).transferFrom(listing.seller, msg.sender, listing.tokenId);
